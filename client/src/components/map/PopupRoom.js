...
import { Pagination, Autoplay } from "swiper/modules"; // Bỏ Lazy khỏi modules
import "swiper/css";
  ...
  return (
    <Card sx={{ maxWidth: 400 }}>
      <ImageListItem sx={{ display: "block" }}>
        ...
        />
        <Swiper
          modules={[Autoplay, Pagination]} // Bỏ Lazy khỏi modules
          autoplay
          ...
          {images.map((url) => (
            <SwiperSlide key={url}>
              <Box
                ...
                src={url}
                alt="room"
                loading="lazy" // Thêm thuộc tính loading="lazy"
                sx={{
                  height: 255,
                  display: "block",
                  overflow: "hidden",
